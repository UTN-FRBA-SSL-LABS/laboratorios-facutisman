...
> R: Setea la cantidad en 0, es importante porque al crear un carrito nuevo este no deberia tener ningun producto.
...
> R:< es el incorrecto, deberia ser <=
...
> R:29 y 30, no se ejecutaron porque no se hizo ningun test en test_unitario de ese caso, se hicieron en test_integracion
...
> R: El test unitario prueba funciones especificas de forma indivudal, en cambio el de integracion hace operaciones, llama funciones y prueba en un contexto mas completo. El bug de carrito_total lo detectó primero el unitario. 
...
> R:Es peligroso en C porque podria acceder a una porcion de memoria que no le corresponde. En python o en java esto no sucede porque gestionan la memoria de forma automatica, no manual. Por lo que se lanzaria una interrupcion. 
...
> R:Que podemos verificar exactamente lo que esperamos, frente a lo que verdaderamente hace el programa. Podria pasar que viendo el codigo solo no sepas exactamente que devuelve el programa. 
...
> R: Porque el bug estaba en multiplicar el total de cada producto por su cantidad, por lo que si el prodcuto solo era 1, el bug pasaba silencioso. 
